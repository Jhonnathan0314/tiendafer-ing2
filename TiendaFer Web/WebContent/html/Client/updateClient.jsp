...
<body class="background">
        ...
        <form action="Servlet?menu=Client" method="post" class="container">
	        	<a href="Servlet?menu=Client&action=searchAll">
                ...
                <h3 class="textField">Ingrese nombre actual del cliente</h3>
                <div class="select">
	                <select class="selectText" name="dniCliente">
		        		<option selected> CLIENTE ACTUAL </option>
		        		<c:forEach var="cliente" items="${clients }">
			            	<option value="${cliente.getDni() }">${cliente.getName() }</option>
		        		</c:forEach>
		            	
		        	</select>
				</div>

                <h3 class="textField">Ingrese el nuevo nombre del cliente</h3>
                <input type="text" class="dataField" name="txtNewClientName" placeholder="Nuevo nombre">
