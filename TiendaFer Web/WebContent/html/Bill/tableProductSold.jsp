...
<body class="background">
		...
		<form action="Servlet?menu=Bill" method="post" class="container">
                ...
                <table class="peTable">
                    ...
                    <tbody>
                    	<c:forEach var="productSold" items="${productSolds }">
	                        <tr class="scroll">
	                            <th>${productSold.getProduct().getName() }</th>
	                            <th>${productSold.getBill().getBillNumber() }</th>
	                            <th>${productSold.getProductAmount() }</th>
	                            <th>${productSold.getUnitValue() }</th>
	                            <th>${productSold.getTotalValue() }</th>
	                        </tr>
	                    </c:forEach>
                    </tbody>
                </table>
